...
comes with it and then multiplies every inner probability with <span
class="fixed">p</span>, returning a list of pairs of items and
...
a default context with <span class="fixed">return</span> and then <span
class="fixed">fmap</span> a function over that and flatten the resulting
probability list, every probability that results from the function would
be multiplied by the <span class="fixed">1%1</span> probability that we
made with <span class="fixed">return</span>, so it wouldn't affect the
context. The reasoning for <span class="fixed">m \>\>= return</span>
being equal to just <span class="fixed">m</span> is similar. The third
law states that <span class="fixed">f \<=\< (g \<=\< h)</span> should be
the same as <span class="fixed">(f \<=\< g) \<=\< h</span>. This one
holds as well, because it holds for the list monad which forms the basis
of the probability monad and because multiplication is associative.
<span class="fixed">1%2 \* (1%3 \* 1%5)</span> is equal to <span
class="fixed">(1%2 \* 1%3) \* 1%5</span>.
